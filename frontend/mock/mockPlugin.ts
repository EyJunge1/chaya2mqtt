...
export async function handleApi(req: IncomingMessage, res: ServerResponse): Promise<boolean> {
  ...
  if (path === "/api/settings" && method === "POST") {
    ...
    const displayDark = params.get("display_dark");
    if (displayDark === "1" || displayDark === "true") state.displayDark = true;
    else if (displayDark === "0" || displayDark === "false") state.displayDark = false;
