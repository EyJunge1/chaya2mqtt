<script lang="ts">
  ...
  import { ACTIVE_ACCENT, HOVER_SURFACE, ICON_WELL } from "../ui/styles.ts";
  ...
  const apNavItems: NavItem[] = [
    { to: "/", labelKey: "nav.setup", icon: settingsNavItems[0].icon },
  ];
...
<div class="min-h-screen bg-bg lg:flex lg:h-dvh lg:min-h-0 lg:overflow-hidden">
  {#if !hideChrome}
    <aside
      class={cn(
    ...
    >
      ...
      >
        {#if mode === "ap"}
          {#each apNavItems as item (item.to)}
            {@render NavItemLink(item, collapsed, false)}
          {/each}
        {:else}
  ...
  <div class="flex min-w-0 flex-1 flex-col lg:h-full lg:overflow-hidden">
    <header
      class="sticky top-0 z-30 shrink-0 border-b border-border bg-bg/95 pt-[max(0.75rem,env(safe-area-inset-top))] pb-3 backdrop-blur"
    >
      <div class="flex w-full items-center gap-3 px-3 lg:px-4">
        {#if !hideChrome}
    ...
    <div class="min-h-0 flex-1 lg:overflow-y-auto">
      <main
        class="mx-auto w-full max-w-3xl px-4 py-4 pb-[max(4.5rem,calc(env(safe-area-inset-bottom)+3.5rem))] lg:px-6 lg:pb-8"
      >
        {@render children()}
      </main>
    </div>
        ...
        {#if mode === "ap"}
          <div class="mx-auto flex max-w-3xl gap-1">
            {#each apNavItems as item (item.to)}
              ...
              <Link href={item.to} end class={mobileNavClass(pathActive(item.to, true))}>
