#!/usr/bin/env python3
"""Print FLASH/RAM for the release image and the delta vs main."""
...
MARKER = "<!-- chaya2mqtt-firmware-size -->"
TARGETS = ("esp32s3-release",)


@dataclass(frozen=True, slots=True)
class Sizes:
    """FLASH bytes from firmware.bin and static RAM from the ELF."""
...
def render(
    ...
    lines = [MARKER, "## Memory usage (PR vs main)", ""]
    short = commit[:12]
    if baseline and baseline_commit:
        lines.append(f"Compared against main `{baseline_commit[:12]}`. Current `{short}`.")
    elif baseline:
        lines.append(f"Compared against the last main baseline. Current `{short}`.")
    else:
        lines.append(f"No main baseline yet. Current `{short}`.")
    lines += [
        "",
...
def load_baseline(path: str) -> tuple[str | None, dict[str, Sizes]]:
    ...
    reports: dict[str, Sizes] = {}
    for name in TARGETS:
        flash = raw.get(f"{name}.flash")
        ram = raw.get(f"{name}.ram")
