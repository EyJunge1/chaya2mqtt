...
| **Charge** | ETA6098 from USB-C; [FAQ](https://docs.waveshare.com/ESP32-S3-ePaper-1.54G/FAQ): ~30 min to 4.1 V, ~44 min full | No firmware |
...
Waveshare low-power FAQ (factory examples, ~1 mA): ~4 days waking every 60 s, ~10 days if left in low-power. **Chaya keeps Wi-Fi up**, so those numbers do not apply — expect hours, not days, until we add sleep.
