...
Playwright once: `cd frontend && npx playwright install chromium`. clang-tidy / clang-format **23** are required in CI (`scripts/ci_install_clang_tools.sh`); locally `make check-firmware-tests` skips them if missing.
