...
| Battery / audio gates / queue coalescing / display link + battery icons | pure helpers in `hw`, `audio`, `async`, `display` | `test_hw` | Settings, dashboard |
...
2. **AP setup**—scan the WIFI QR on the display (phone camera); join WPA2/WPA3 `Chaya2MQTT`; confirm `chaya2mqtt.local` and captive probes reach the connected AP; SSID scan; test & connect; commit.
...
5. **MQTT pairing/telemetry**—broker + partner; LWT online; send/receive heart; while MQTT is down, modem power saving is off (`WIFI_PS_NONE`), and after connection it returns to `MIN_MODEM`.
