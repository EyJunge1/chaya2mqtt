...
inline bool mqttPublishAckBegin(MqttPublishAckState* state, int messageId,
                                uint32_t clientGeneration, int expectedCounter) {
    if (state == nullptr || messageId < 0
...
inline bool mqttPublishAckConfirm(MqttPublishAckState* state, int messageId,
                                  uint32_t clientGeneration) {
    if (state == nullptr || state->status != MqttPublishAckStatus::Pending
        || state->messageId != messageId || state->clientGeneration != clientGeneration) {
        return false;
    }
...
inline bool mqttPublishAckIsPending(const MqttPublishAckState& state) {
    return state.status == MqttPublishAckStatus::Pending;
}

inline bool mqttPublishAckWasConfirmed(const MqttPublishAckState& state, int messageId,
                                       uint32_t clientGeneration) {
