...
void wlanRecoveryServiceLoop() {
    ...
    case WlanRecoveryAction::ForcedReassoc:
        ESP_LOGW(TAG, "WLAN recovery action=ForcedReassoc downFor=%lu ms otaBlock=%d restarts=%u",
                 downFor, otaBlock ? 1 : 0, static_cast<unsigned>(restartsUsed));
        wlanForceStaReassoc("recovery");
        break;
    case WlanRecoveryAction::Restart:
        ESP_LOGW(TAG, "WLAN recovery action=Restart downFor=%lu ms otaBlock=%d restarts=%u", downFor,
                 otaBlock ? 1 : 0, static_cast<unsigned>(restartsUsed));
        recoveryNoteRestart();
        webServerEnd();
