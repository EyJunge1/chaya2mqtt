...
bool appendDeviceJsonObject(char *body, size_t bodyLen, size_t *pos) {
    ...
    int n =
        snprintf(body + *pos, bodyLen - *pos, "{\"hostname\":\"%s\",\"version\":\"%s\",\"mode\":\"%s\",\"deviceId\":", hostname,
                 APP_VERSION, ap ? "ap" : "sta");
    ...
    *pos += static_cast<size_t>(n);
    ...
    n = snprintf(body + *pos, bodyLen - *pos, ",\"batteryMv\":%d,\"batteryPct\":%d}", batteryMilliVolts(), batteryPercent());
    if (n < 0 || *pos + static_cast<size_t>(n) >= bodyLen) {
        return false;
    }
    *pos += static_cast<size_t>(n);
...
void handleApiCsrfGet(AsyncWebServerRequest *req) {
    ...
    char body[96];
    const int n = snprintf(body, sizeof(body), "{\"token\":\"%s\",\"expiresInSeconds\":%lu}", token,
...
void handleApiBootstrapGet(AsyncWebServerRequest *req) {
    ...
    int n = snprintf(body, sizeof(body), "{\"csrf\":{\"token\":\"%s\",\"expiresInSeconds\":%lu},\"device\":", token,
                     static_cast<unsigned long>(expiresInSeconds));
    if (n < 0 || static_cast<size_t>(n) >= sizeof(body)) {
        sendErr(req, 500, "json");
        return;
    ...
    if (sta) {
        ...
        n = snprintf(body + pos, sizeof(body) - pos,
                     ...
                     rx, tx, mqttIsConnected() ? "true" : "false", configured ? "true" : "false", paired ? "true" : "false",
        ...
        n = snprintf(body + pos, sizeof(body) - pos, ",\"chaya\":null,\"mqtt\":null,\"update\":null,\"settings\":null");
        if (n < 0 || pos + static_cast<size_t>(n) >= sizeof(body)) {
            sendErr(req, 500, "json");
            return;
        }
