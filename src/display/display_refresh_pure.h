 ...
 */
inline unsigned long displayHeartRedrawWaitMs(unsigned long nowMs, unsigned long lastEnqueueMs,
                                              unsigned long minIntervalMs, bool pending) {
    if (!pending) {
        return ULONG_MAX;
    }
...
inline bool displayHeartNeedsFollowUpRedraw(int drawnRx, int drawnTx, int currentRx,
                                            int currentTx, bool iconChanged,
                                            bool batteryIconChanged, bool hadPending) {
    return hadPending || iconChanged || batteryIconChanged || currentRx != drawnRx
           || currentTx != drawnTx;
}
